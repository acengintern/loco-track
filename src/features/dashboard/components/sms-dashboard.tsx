          ...
          <div className="mt-4 space-y-3">
            {data.readyForClient.length === 0 ? (
              ...
              data.readyForClient.map((p) => (
                <div
                  key={p.projectId}
                  className="flex items-center justify-between rounded-md border border-primary/20 bg-primary/5 p-3.5 text-xs"
                >
                  <div>
                    <p className="font-semibold text-foreground">{p.projectName}</p>
                    <p className="text-muted-foreground text-[11px]">{p.brandName}</p>
                  </div>
                  <Link
                    href={`/projects/${p.projectId}`}
                    className="inline-flex items-center gap-1 rounded-md bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground hover:bg-primary/90 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
                  >
          ...
          <div className="mt-4 space-y-3">
            {data.readyToPublish.length === 0 ? (
              ...
              data.readyToPublish.map((p) => (
                <div
                  key={p.projectId}
                  className="flex items-center justify-between rounded-md border border-emerald-500/20 bg-emerald-500/5 p-3.5 text-xs"
                >
                  <div>
                    <p className="font-semibold text-foreground">{p.projectName}</p>
                    <p className="text-muted-foreground text-[11px]">{p.brandName}</p>
                  ...
                  <Link
                    href={`/projects/${p.projectId}`}
                    className="inline-flex items-center gap-1 rounded-md bg-emerald-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-emerald-700 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
                  >
                    Publikasi <ArrowRight className="size-3" />
                  </Link>
                </div>
              ))
            )}
            ...
            {data.activeClientRevisions.length === 0 ? (
              ...
              data.activeClientRevisions.map((rev) => (
                ...
                >
                  ...
                  <p className="text-muted-foreground text-[11px]">{rev.projectName}</p>
        ...
        >
          <div className="flex items-center justify-between border-b border-border pb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href="/projects"
                className="inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring rounded-sm pl-1"
              >
